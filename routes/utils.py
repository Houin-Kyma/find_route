...
def dfs_paths(graph, start, goal):
    """Функция поиска всех возможных маршрутов из одного
    города в другой. Вариант посещения одного и того же города более одного раза, не рассматривается"""
    stack = [(start, [start])]
    while stack:
...
def get_routes(request, form) -> dict:
    ...
    if not len(all_ways):
        # Нет ни одного маршрута, через который можно проехать
